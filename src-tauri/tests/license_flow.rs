...
// 用开发私钥在测试内自签（等价于 bidguard-keygen issue），既验证流程，也充当
// 「keygen 令牌格式 == app 验签格式」的契约测试。
//
// 需私钥种子（base64url）：环境变量 BIDGUARD_DEV_PRIV。未设置则跳过（CI 无私钥不失败）。
//   BIDGUARD_DEV_PRIV=<seed> cargo test --test license_flow -- --nocapture
use bidguard_lib::db;
...
fn issue(sk: &SigningKey, machine_code: &str, plan: &str, uses: Option<u64>, days: Option<i64>) -> String {
    ...
    let header = serde_json::json!({ "v": 1, "alg": "Ed25519", "kid": "lic-dev-2026a" });
...
fn counted_license_consume_exhaust_refund() {
    ...
    let lic = issue(&sk, &code, "counted", Some(3), None);
...
fn machine_mismatch_rejected() {
    ...
    let lic = issue(&sk, &bogus, "counted", Some(5), None);
