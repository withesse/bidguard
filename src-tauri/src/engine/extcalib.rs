...
// 概率校准不在本模块：生产实现见 `engine::calibrate`（Platt + 共形三带，
// 参数落 fixtures/calibration/score_calib.json）。本模块只做【外部真值上的评估】，
// 与之互补——calibrate 负责拟合与线上分带，extcalib 负责用非同源语料检验刻度是否可信。
// 曾在此另写过一套 Platt/isotonic，因与 calibrate 重复而移除。
