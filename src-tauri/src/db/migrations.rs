...
// V16：补入官方标准范本的「投标文件格式」条款作为查重源样板。
...
const OFFICIAL_TEMPLATES_V16: &str = "
...
mod tests {
    ...
    fn migrates_fresh_db_and_is_idempotent() {
        ...
        let tpl: i64 = conn
            ...
            .unwrap();
        assert_eq!(tpl, 9, "应内置 3 条默认模板 + 6 条官方范本表单");
        let official: i64 = conn
            ...
            .unwrap();
        assert_eq!(official, 6, "V16 应补入 6 条官方范本表单");
    }
